...
void main() {
    ...
    vec2 newTextureCoord = uTexScale * (aTextureCoord + vec2(1.0 * uTimeScale, 1.0 * uTimeScale));

	vec4 heightColor = texture2D(uSampler2, newTextureCoord);
	vec3 newVertex = aVertexPosition;

	newVertex.y += heightColor.r * heightColor.a * uHeightScale;

    // Transformed Vertex position
    vec4 vertex = uMVMatrix * vec4(newVertex, 1.0);
